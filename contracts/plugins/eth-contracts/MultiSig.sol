...
contract MultiSig {
    ...
    event valid_function_sign(
        string name,
        uint64 id,
        uint64 current_signed_number,
        uint propose_height,
        bytes32 hash
    );
    ...
    constructor(address[] memory s) enough_signers(s) {
    ...
    function is_all_minus_sig(
        ...
        for (uint i = 0; i < invoke.invoke_signers.length; i++) {
            require(
                join_height < invoke.propose_height,
        ...
        valid_invoke_num++;
        emit valid_function_sign(
            name,
            id,
            uint64(valid_invoke_num),
            invoke.propose_height,
            hash
        );
    ...
    modifier only_multisig_proxy() {
        require(
            enabled_proxies[msg.sender],
            "only an auth proxy can call this"
        );
        _;
    }

    function add_multisig_proxy(
        uint64 id,
        address proxy
    ) public only_signer is_majority_sig(id, "add_multisig_proxy") {
        enabled_proxies[proxy] = true;
    }

    function is_multisig_proxy_enabled(
        address proxy
    ) public view returns (bool) {
        return enabled_proxies[proxy];
    }

    function remove_multisig_proxy(
        uint64 id,
        address proxy
    ) public only_signer is_majority_sig(id, "remove_multisig_proxy") {
    ...
    function update_and_check_reach_majority(
        ...
        bytes32 hash,
        address sender
    ) public only_multisig_proxy returns (bool) {
        uint minority = signer_number - get_majority_number();
        if (!is_all_minus_sig(minority, id, name, hash, sender)) return false;
        set_called(hash);
        return true;
    }
    ...
    modifier is_majority_sig(uint64 id, string memory name) {
        ...
        uint minority = signer_number - get_majority_number();
        if (!is_all_minus_sig(minority, id, name, hash, msg.sender)) return;
        set_called(hash);
        _;
    }
        ...
        is_majority_sig(id, "reform_signers")
    {
        ...
        for (uint i = 0; i < s.length; i++) {
            if (array_exist(old_signers, s[i])) {} else {
                signer_join_height[s[i]] = block.number;
            }
        }
        for (uint i = 0; i < old_signers.length; i++) {
            if (array_exist(s, old_signers[i])) {} else {
