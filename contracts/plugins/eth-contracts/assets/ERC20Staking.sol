...
import {TokenInterface} from "../erc20/TokenInterface.sol";
...
contract ERC20Staking is Ownable, ReentrancyGuard {
    ...
    constructor(address _target_token, address _lp_token) {
    ...
    function stake(uint256 _amount) public nonReentrant returns (uint256) {
        ...
        IERC20(target_token).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        ...
        uint256 lp_amount = 0;
        {
            if (IERC20(lp_token).totalSupply() == 0) {
                require(
                    amount >=
                        uint256(10) ** IERC20Metadata(target_token).decimals(),
                    "too small target token amount"
                );
                lp_amount = amount
                    .safeMul(uint256(10) ** IERC20Metadata(lp_token).decimals())
                    .safeDiv(
                        uint256(10) ** IERC20Metadata(target_token).decimals()
                    );
            } else {
        ...
        if (IERC20(lp_token).totalSupply() != 0) {
            uint256 expect_amount = lp_amount.safeMul(prev).safeDiv(
    ...
    function claim(uint256 _amount) public nonReentrant returns (uint256) {
        uint256 total = IERC20(lp_token).totalSupply();

        require(
            IERC20(lp_token).balanceOf(msg.sender) >= _amount,
            "not enough lp token to claim"
        );
        ...
        uint256 amount = 0;
        {
            if (IERC20(lp_token).totalSupply() == 0) {
                ...
                amount = _amount
                    .safeMul(IERC20(target_token).balanceOf(address(this)))
                    .safeDiv(total);
            }
        }
    ...
    function getPricePerFullShare() public view returns (uint256) {
        return
            IERC20(target_token)
                ...
                .safeMul(1e18)
                .safeMul(uint256(10) ** IERC20Metadata(lp_token).decimals())
                .safeDiv(IERC20(lp_token).totalSupply())
                .safeDiv(
                    uint256(10) ** IERC20Metadata(target_token).decimals()
                );
    }
