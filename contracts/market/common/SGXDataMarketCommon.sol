...
contract SGXDataMarketCommon is Ownable, GasRewardTool, SGXProxyBase {
    ...
    ) public rewardGas returns (bytes32) {
        bytes32 vhash;
        IMarketCommon dl = IMarketCommon(data_lib_address);
        {
            bytes memory data = abi.encodeWithSelector(
                dl.createStaticData.selector,
                _hash,
                _data_uri,
                _price,
                _pkey,
    ...
    function transferRequestOwnership(
        ...
        IMarketCommon dl = IMarketCommon(data_lib_address);
        bytes memory data = abi.encodeWithSelector(
            dl.transferRequestOwnership.selector,
            _vhash,
            request_hash,
            new_owner
        );
    ...
    event SDMarketChangeRequestRevokeBlockNum(
        bytes32 indexed vhash,
        uint256 block_num
    );
    ...
    function changeRequestRevokeBlockNum(
        ...
        require(
            ...
            "only data owner may change request block number"
        );
        IMarketCommon dl = IMarketCommon(data_lib_address);
        {
            bytes memory data = abi.encodeWithSelector(
                dl.changeRequestRevokeBlockNum.selector,
                _vhash,
                _new_block_num
            );
            market.delegateCallUseData(data_lib_address, data);
